...
That means if you have a AR model which called like a some Faker class and field which called like a some faker method in that class 
a value will be generated by Faker gem. 

For example: Model `Superhero` has the `name` and `power` fields, so the value will be generated by `Faker::Superhero.name` and `Faker::Superhero.power`. [Superhero doc](https://github.com/stympy/faker/blob/master/doc/superhero.md).
```
